...
def veri_cek_ve_yaz():
    ...
    label.config(text="Veriler çekildi.")
    progress_bar.stop()
...
def grafikleri_oluştur_ve_goster():
    ...
    # Grafik 3: KDE plot
    plt.figure(figsize=(12, 8))
    sns.kdeplot(indirimli_fiyatlar, label='İndirimli Fiyatlar', fill=True)
    sns.kdeplot(normal_fiyatlar, label='Normal Fiyatlar', fill=True)
    plt.xlabel('Fiyat')
    plt.ylabel('Yoğunluk')
    plt.title('Kitap Fiyatları')
    plt.legend()
    ...
    plt.title('Toplam Fiyat Dağılımı')
    plt.show()
...
window = tk.Tk()
window.title("Kitap Fiyatları Analizi")
window.geometry("400x200")

label = Label(window, text="Veri çekmek veya grafik oluşturmak için ilgili düğmeye tıklayın.")
label.pack(pady=10)

button_retrieve = Button(window, text="Veri Çek", command=retrieve_data)
button_retrieve.pack(pady=5)

button_graphs = Button(window, text="Grafikler Oluştur", command=create_graphs)
